...
def register():
    ...
    if not all((login, name, pasword)):
        return render_template('regiser.html', message='Все поля должны быть заполнены')
    db_sess = db_session.create_session()
    user = db_sess.query(users.Users).filter(users.Users.login == login).first()
    if user:
        return render_template('register.html', message='Такой логин уже используют')
    user = db_sess.query(users.Users).filter(users.Users.name == name).first()
    if user:
        return render_template('register.html', message='Такое прозвище уже существует')
    making = functions.make_new_user({'login': login, 'name': name, 'password': pasword})
    if making != 'Удачно':
        return render_template('register.html', message=making)
    return redirect('/my_wall')
